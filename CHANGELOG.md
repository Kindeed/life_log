...
#### 变更 (Changed)
- 应用版本升级到 `1.4.28+34`。
- “更多”重构为“记录与分析 / 工具 / 应用”三个二级功能分组，统一入口布局并减少重复结构。
- 统计面板、订阅管理、全部费用记录、遥测计算器、外观设置、数据管理和关于应用统一归属“更多”目录。
- 个人页更名为“账户与同步”，只保留账户、同步状态和开发者入口。

#### 修复 (Fixed)
- 修复统计、外观、数据管理、遥测和关于入口同时出现在“更多”和个人页造成的重复与信息架构混乱。
- 保留并回归验证订阅删除链路，避免本次目录重构影响此前的删除修复。

#### 验证 (Validation)
- 本地 `flutter test`：594 项通过。
