...
final class LoadExpenseRecordEntries {
  final ExpenseRecordRepositoryPort _repository;

  const LoadExpenseRecordEntries(this._repository);

  Future<AppResult<List<ExpenseRecordEntry>>> call() async {
    try {
      return AppResult.success(await _repository.getAllEntries());
    } catch (error, stackTrace) {
      return AppResult.failure(
        AppFailure(
          code: 'expense-record/load-entries',
