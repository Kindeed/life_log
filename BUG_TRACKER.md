...
| U64 | High | fixed | `TelemetryCalcDetailView` / `rate_bandwidth` | `码率与带宽` 页面仍表现为旧式上下割裂布局，公式依据在结果和输入之后，用户先看到结果卡片但看不到计算关系。 | Fixed: formula references now render at the top of the same calculation workbench, before result and input sections, with a styled selectable formula block. |
...
| U67 | Medium | fixed | `TelemetryCalcDetailView` input consistency | 其他遥测计算详情页仍可能混用独立选择卡片、普通文本框和数值 tile，导致公式、结果、输入虽在同页但视觉语言不统一。 | Fixed: number, select, and expression inputs now share the same compact input-tile surface, and widget tests iterate all calculator detail pages for formula/result/input ordering. |
...
| U70 | Medium | fixed | `TelemetryCalcDetailView` reference alignment | 参考稿 B 强调输入/输出工作台先行、公式作为轻量依据，但实装版仍把公式作为首屏大区块，导致真机效果和预览稿不一致。 | Fixed: detail pages now render the input/output workbench before the support formula section, keep wide layouts as input-left/output-right, and reduce formula content to a compact trailing support panel. |
| U71 | Medium | fixed | `TelemetryCalcDetailView` mobile workbench | 真机验证发现移动端若把完整输入表单放在输出前，输出摘要会被挤出首屏，底部“恢复默认 / 保存模板”按钮还会出现轻微纵向 overflow。 | Fixed: mobile workbench now uses compact two-column input/output panes, keeps formulas as trailing support content, and moves reset/save template actions into the AppBar so the linked input/output space stays intact. |
| U72 | Medium | fixed | `TelemetryCalcDetailView` reference alignment | 用户确认现版仍未满足“B：左输入右输出工作台，短标题 + 副标题，输入和结果空间上绑定，并吸收 A 的主结果强调”，因为移动端仍是上下堆叠而不是空间绑定。 | Fixed: the detail workbench now renders input on the left and output on the right even on mobile, with compact editable input rows and a compact emphasized primary-result panel. |
| U73 | Medium | open | `TelemetryCalcDetailView` calculator-specific balance | 新增计算模块若输入参数明显多于输出项，当前固定左输入右输出双栏会显得左侧过重、右侧过空，影响详情页协调感和专业感。 | Proposed: evaluate calculator-aware layout treatment such as strengthening the output pane with decision/status context, moving sparse outputs into a top summary band, or grouping/collapsing dense inputs while preserving local-only calculation behavior. |
