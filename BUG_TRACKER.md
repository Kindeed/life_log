...
| U73 | Medium | fixed | `TelemetryCalcDetailView` calculator-specific balance | 新增计算模块若输入参数明显多于输出项，当前固定左输入右输出双栏会显得左侧过重、右侧过空，影响详情页协调感和专业感。 | Fixed: output panes now use stronger surface treatment and include a compact `工程判断` insight block derived from existing calculation outputs, while preserving the left-input/right-output workbench and local-only calculation flow. |
