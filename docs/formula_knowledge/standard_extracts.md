...
| BOOKANT-009 | Balanis aperture-antenna and Fourier-transform aperture-field material | Continuous aperture taper efficiency, aperture-field Fourier transform, edge taper, beam efficiency, and sampled-pattern integration | Catalog adds RF-086 to RF-092. UI should let measured/simulated pattern data override ideal circular-aperture approximations. |
| BOOKANT-010 | Balanis array-synthesis chapters and Mailloux phased-array taper design | Discrete amplitude taper efficiency, taper loss, sidelobe level, null depth, and Dolph-Chebyshev/Taylor taper generation | Catalog adds RF-093 to RF-098. Treat taper synthesis weights as generated artifacts with validation plots, not as hand-entered scalar constants. |
| BOOKANT-011 | Balanis mutual-impedance chapters and Mailloux active-array network treatment | Active reflection coefficient, active VSWR, accepted array power, and coupling/reflection efficiency from the array S-parameter matrix and excitation vector | Catalog adds RF-099 to RF-102. These formulas are essential for scan-angle warnings because passive single-port VSWR can hide active mismatch. |
| BOOKANT-012 | Balanis antenna-measurement chapters | Gain-sum observables, three-antenna gain measurement, and gain-transfer measurement with a calibrated reference antenna | Catalog adds RF-103 to RF-105. These belong in a measurement workbench and should carry range/polarization/equal-geometry assumptions. |
| BOOKANT-013 | Balanis reflector-antenna geometry and DESCANSO reflector efficiency context | Parabolic reflector focal ratio, rim depth, feed edge angle, illuminated diameter, central blockage ratio, and approximate blockage efficiency | Catalog adds RF-106 to RF-112. These outputs let the antenna card show geometry and blockage tradeoffs instead of only gain. |
| BOOKANT-014 | Balanis feed illumination and DESCANSO antenna-temperature context | Spillover efficiency integrates feed power pattern over the reflector subtended angle; spillover noise uses sky and warm-ground/structure temperatures | Catalog adds RF-113 and RF-114. Treat feed pattern integration as a procedure with measured or modeled feed data. |
| BOOKANT-015 | Balanis polarization-pattern material and ITU antenna-pattern context | Antenna cross-polar discrimination is co-polar gain minus cross-polar gain; dB axial ratio converts to a linear axial ratio | Catalog adds RF-115 to RF-117. Pattern direction, polarization basis, and measurement convention must be explicit. |
| BOOKANT-016 | Mailloux electronically scanned array and Balanis array-aperture material | Projected aperture, scan beam broadening, planar element count, active aperture dimensions, coherent array EIRP, and true-time-delay steering | Catalog adds RF-118 to RF-125. These are practical right-side outputs for array sizing and wideband scan warnings. |
...
| DIGCOM-014 | Sklar/Harris and Proakis AWGN digital modulation performance chapters | Coherent M-PSK, square M-QAM, coherent/noncoherent orthogonal FSK, OOK/ASK, DQPSK, and M-FSK error-probability formulas | Catalog adds BB-121 to BB-131. Treat several M-ary rows as approximations or procedures because accuracy depends on SNR region, labeling, detection type, and constellation normalization. |
| DIGCOM-015 | Goldsmith wireless channel capacity/diversity chapters; Proakis fading-channel performance material | Fading-channel BER is averaged over instantaneous SNR; MRC sums branch SNRs, selection combining uses the maximum branch SNR, and iid Rayleigh outage has closed forms for MRC/SC | Catalog adds BB-132 to BB-137. These belong in a stochastic-channel result group next to average SNR/outage, not in the deterministic link-budget subtotal. |
| DIGCOM-016 | Sklar/Harris and Proakis channel-coding chapters | Coding gain is the Eb/N0 reduction at a fixed target error probability; generic block-code rate is `k/n`; bounded-distance correction capability is `floor((d_min-1)/2)` | Catalog adds BB-138 to BB-140. CCSDS-specific RS/LDPC/Turbo tables still override generic textbook code parameters when a standard mode is selected. |
| DIGCOM-017 | Sklar/Harris and Proakis block-code and R-S coding chapters | Detectable/correctable error counts derive from minimum distance; bounded-distance success and failure probabilities use binomial sums; R-S symbol-error probability follows from grouped input bit errors | Catalog adds BB-141 to BB-149. Use these for engineering estimates and R-S interleaver output cards; standards still own exact R-S shortening/interleaving mode choices. |
| DIGCOM-018 | Proakis and Sklar/Harris coding-performance chapters | Pairwise error probability and code spectra drive union bounds; convolutional-code performance can be approximated by truncated free-distance spectra; Viterbi traceback depth is an implementation rule of thumb | Catalog adds BB-150 to BB-160. Use exact published or measured distance spectra/tables when available; do not infer CCSDS performance curves from generic bounds alone. |
| DIGCOM-019 | Sklar/Harris and Mengali/D'Andrea receiver implementation material | Sampling rate, sample period, samples per frame, normalized carrier frequency offset, per-symbol/per-sample phase rotation, timing error in UI, and sample-epoch quantization jitter | Catalog adds BB-161 to BB-169. These are receiver diagnostic outputs that make sampling and acquisition choices visible. |
| DIGCOM-020 | Gardner and Mengali/D'Andrea synchronization-loop material | NCO phase increment/frequency conversion, loop update interval, update-normalized loop bandwidth, loop-coefficient design placeholder, instantaneous phase, and wrapped phase error | Catalog adds BB-170 to BB-176. Exact loop coefficients remain architecture-dependent and must be generated by reviewed code, not copied as one universal formula. |
| DIGCOM-021 | Proakis/Sklar synchronization and detection material | Sliding correlation, normalized synchronization metric, sync peak margin, and false-alarm-threshold procedure for known preambles or sync markers | Catalog adds BB-177 to BB-180. Thresholds depend on the selected noise-only metric distribution and acquisition search model. |
| DIGCOM-022 | Goldsmith outage-capacity and Rappaport lognormal-shadowing material | Target Rayleigh outage can be inverted for required average SNR; lognormal shadowing percentiles convert shadowing standard deviation into fade depth and percentile path loss | Catalog adds LINK-158 to LINK-162 for channel availability and margin result cards. |
| DIGCOM-023 | Goldsmith/Rappaport channel selectivity material; Proakis OFDM guard-interval context | Frequency-selectivity ratio, flat-fading condition, time-selectivity ratio, quasi-static block condition, coherence block size, Doppler spread, normalized Doppler, delay-spread ISI ratio, and cyclic-prefix margin | Catalog adds LINK-163 to LINK-171. These should be warnings/diagnostics attached to the channel model, not hidden assumptions. |
| DIGCOM-024 | Rappaport/Goldsmith fade-dynamics material | Rician K conversion, Rayleigh median power gain, level-crossing rate, and average fade duration | Catalog adds LINK-172 to LINK-176 for stochastic channel dynamics beyond average outage probability. |
...
| DS-OPT-005 | Chapter 2.2.2.3, equations 2.2-6 and 2.2-7 | Diffuse sky-background power is driven by sky radiance, effective receiver area, FOV solid angle, optical bandpass, and receive efficiency; point-source background omits the FOV solid-angle term and uses source spectral irradiance | Catalog adds OPT-021 to OPT-023. This directly supports a separate right-side result group for sky, point-source, solar, and dark-count background contributors. |
| DS-OPT-006 | Chapter 2.2.3.3 and chapter 3.4.2 | Diffraction-limited FOV is about `2.44 lambda/D`; atmospheric turbulence redistributes energy over an angular region of order `lambda/r0`; focal-plane spot diameter uses `2.44*f*lambda/D` in the diffraction limit and replaces `D` with `r0` under seeing | Catalog adds OPT-024 and OPT-027 to OPT-030. UI should expose a receive-FOV trade card rather than hiding the turbulence penalty in one loss field. |
| DS-OPT-007 | Chapter 3.4.1 to 3.4.2 | Optical turbulence is represented through the refractive-index structure function `D_n(r)=C_n2*r^(2/3)` and the Fried coherence diameter integral over the `C_n2` profile | Catalog adds OPT-025 to OPT-028. Full Hufnagel-Valley/CLEAR profile tables remain a procedure/data-asset task. |
| DS-OPT-008 | Chapter 6.1.2 and 6.2.2 | Detector-array receivers can select or combine focal-plane elements so the selected signal and background counts drive the PPM error probability under turbulence-distorted spots | Catalog adds OPT-031 to OPT-033. Treat as an optimization workbench: selected pixels, signal sum, background sum, and predicted symbol error. |
| CCSDSOPT-001 | CCSDS search/active-publication entries for 141.0-B-2, March 2026 | Non-coherent optical physical layer covers optical carrier/linewidth/signaling constraints; first-pass formulas use `f=c/lambda`, small wavelength-frequency spacing, coherence time/length, OOK slot rate, duty cycle, and peak/average power | Catalog adds OPT-049 to OPT-057. Exact mandated carrier frequencies, linewidth limits, data-rate options, and waveform tables remain table assets. |
| CCSDSOPT-002 | CCSDS search/active-publication entries for 142.0-B-2, March 2026, cross-checked with public implementation descriptions of the HPE/SCPPM chain | Non-coherent optical coding/synchronization maps transfer-frame plus CRC/termination through coding, interleaving/accumulation, PPM mapping, guard slots, repetition, and synchronization structures | Catalog adds OPT-058 to OPT-068. Use these as structural efficiency formulas only; exact code parameters, sync markers, randomizer details, and coded-performance curves remain extraction tasks. |
| CCSDSOPT-003 | CCSDS 142.10-O-1 Reed-Solomon Product Code for Optical Communication | Rectangular product code dimensions multiply row and column R-S dimensions: code symbols `n_row*n_col`, information symbols `k_row*k_col`, and rate as their ratio | Catalog adds OPT-069 to OPT-073. Exact selected code rows, interleaving, and decoder performance examples remain table/procedure tasks. |

## ITU-R Free-Space Optical Propagation Extracts

Source: ITU-R P.1814-1, `Prediction methods required for the design of terrestrial free-space optical links`, approved September 2025.

| Extract ID | Source location | Formula / rule captured | Implementation implication |
| --- | --- | --- | --- |
| ITUFSO-001 | P.1814-1 annex section 2, equation 1 | FSO link margin subtracts receiver sensitivity, geometrical attenuation, atmospheric attenuation, scintillation attenuation, and system losses from emitter power | Catalog adds OPT-034. This is a top-level FSO margin card distinct from deep-space optical photon counting. |
| ITUFSO-002 | P.1814-1 annex section 3, equation 2 | Geometrical attenuation uses `10log10(S_d/Scapture)` with `S_d=pi*(d*theta)^2/4`, clipped to zero if receiver capture area exceeds beam area | Catalog adds OPT-035 and OPT-036. Inputs must preserve P.1814's km/mrad convention or convert explicitly. |
| ITUFSO-003 | P.1814-1 annex section 4.1, equations 3 to 10 | Specific atmospheric attenuation decomposes into clear-air and excess terms; visibility/MOR can be converted into suspended-particle specific attenuation, including the wavelength-dependent 0.4-1.55 um power-law and piecewise exponent | Catalog adds OPT-037 to OPT-041. Tables for mid/far-IR coefficients and uncertainties remain data assets. |
| ITUFSO-004 | P.1814-1 annex section 4.2, equations 12 and 13 | Clear-air and suspended-particle path attenuation multiply specific attenuation by path length for short terrestrial FSO paths | Catalog adds OPT-042 and OPT-043. Later implementation should gate by P.1814 validity range and local meteorological data quality. |
| ITUFSO-005 | P.1814-1 annex section 5, equation 20 | Weak-turbulence log-amplitude scintillation variance uses `23.17*k^(7/6)*C_n2*L^(11/6)`; the engineering attenuation allowance is `2*sigma_chi` | Catalog adds OPT-044 and OPT-045. Strong-turbulence saturation and table-based fade-depth examples remain pending. |
| ITUFSO-006 | P.1814-1 annex section 6, equations 21 to 23 | Ambient-light model estimates solar radiated power, a wavelength polynomial for solar spectral power, and received solar power through capture area and receiver optical bandwidth | Catalog adds OPT-046 to OPT-048. This should appear as a background/interference result, not as a link-loss term. |
...
| MEASUNC-008 | NIST/SEMATECH and NIST Dataplot proportion confidence-limit notes | Clopper-Pearson exact interval can be expressed with inverse beta distribution limits; Wilson score interval gives a bounded approximate interval | Catalog adds MEAS-034 to MEAS-037. Use exact bounds for acceptance, with Wilson as a compact engineering summary when sample counts are not tiny. |
| MEASUNC-009 | NIST/SEMATECH binomial model applied to BER/FER demonstration tests | Probability of `k` errors follows `C(N,k)p^k(1-p)^(N-k)`; zero-error and allowed-error pass probabilities come from the binomial CDF | Catalog adds MEAS-038 to MEAS-049. These formulas make test duration, pass probability, and requirement confidence first-class outputs. |
| MEASUNC-010 | Sklar/Proakis AWGN BER relations plus NIST/SEMATECH measured-proportion intervals | Coherent BPSK/QPSK BER relation can be inverted with `Qinv` to infer required `Eb/N0`; compare measured and theory at the same target error probability | Catalog adds MEAS-050 to MEAS-053. Use only as an uncoded AWGN reference; coded links should prefer standard performance tables or measured curves when available. |
...
| ORBSYS-007 | Vallado/Bate first-order perturbation and sun-synchronous orbit design material | J2 secular RAAN, argument-of-perigee, and mean-anomaly rates; sun-synchronous inclination relationship | Catalog adds ORB-038 to ORB-041. Treat as first-order design equations; operational propagation should use validated SGP4 or numerical force models. |
| ORBSYS-008 | Vallado/CelesTrak ground-track and Earth-rotation context | Approximate nodal period, ascending-node longitude spacing, and integer repeat-ground-track closure error | Catalog adds ORB-042 to ORB-044. Repeat candidates need integer search plus longitude normalization and mission-specific tolerance. |
| ORBSYS-009 | Vallado/Bate Kepler propagation material | Epoch mean-anomaly propagation, Kepler's equation, and eccentric-anomaly to true-anomaly conversion | Catalog adds ORB-045 to ORB-047. Solver tolerances and singular cases belong in implementation tests. |
| ORBSYS-010 | SMAD/Vallado contact event solving | Access root function, AOS/LOS elevation-mask crossings, and maximum-elevation event | Catalog adds ORB-048 to ORB-051. These formulas define event semantics; production pass prediction still needs propagated state samples and robust root bracketing. |
| ORBSYS-011 | Vallado/Bate impulsive maneuver chapters | Hohmann transfer impulses/time of flight, pure plane change, and combined speed/plane-change impulse | Catalog adds ORB-052 to ORB-057. UI should show assumptions: two-body, impulsive, coplanar for Hohmann, circular endpoints unless extended. |
| ORBSYS-012 | SMAD/NASA SmallSat communications operations context | Generated data, usable contact time, per-pass capacity, aggregate downlink capacity, storage balance, and contact efficiency | Catalog adds SYS-011 to SYS-018 as system closure formulas that connect orbit access to link budget outputs. |
| ORBSYS-013 | SMAD operations budget context | Required net/line rate, target downlink bits, required compression ratio, queue drain time, energy use, battery depth of discharge, recorder turnover, contact utilization, science return, and light-time | Catalog adds SYS-019 to SYS-030; later app UI should present these as a schedule/workflow summary rather than isolated calculators. |
...
| SATLINK-007 | Maral/Bousquet multi-hop and relay-link budget context | Serial-hop availability, composite outage, and reciprocal multi-hop `C/N0` summation | Catalog adds LINK-177 to LINK-179; use only when hop independence and reference-bandwidth assumptions are explicit. |
| SATLINK-008 | Maral/Bousquet intersatellite and non-equal carrier allocation context, cross-checked with P.525 free-space loss | Inter-satellite free-space loss and non-equal per-carrier power/EIRP allocation | Catalog adds LINK-180 to LINK-182; measured HPA curves and detailed IM products remain scenario data assets. |
...
| Digital comm books | Listed as source; BER/PER/modulation formulas partly seeded | Baseband timing, energy metrics, Shannon/Nyquist, pulse shaping, quantization, matched-filter detection, OFDM, phase jitter, MIMO, shadowing/outage margin, selectivity/coherence diagnostics, and fade-dynamics formulas added | Extract exact CCSDS modulation families and add test vectors for BER, quantization, OFDM, fading, and phase-noise calculators. |
...
| CCSDS-231 | Listed as source; CLTU formulas partly seeded | BCH/LDPC codeword sizing, CLTU start/tail lengths, fill formulas, PLOP duration/efficiency, repeated CLTU timing, and managed parameters extracted | Add mission-specific PLOP managed-parameter tables, maximum CLTU constraints, and validation examples when implementing. |
...
| Maral/Bousquet satellite transponder links | Listed as scenario source; transponder formulas not yet extracted | PFD/SFD/IBO/OBO, transponder input/gain, per-carrier EIRP, cascaded and multi-hop `C/N0`, interference/intermodulation, HPA DC/thermal power, bandwidth/power utilization, non-equal carrier allocation, intersatellite FSPL, and composite availability formulas added | Add saturation curves, measured HPA/intermodulation models, detailed intersatellite relay examples, and validation examples. |
...
| COP1-015 | CCSDS 231 physical-layer operations cross-check | TC PLOP accounting needs acquisition, idle leader, CLTU radiation, idle trailer, repetitions, and inter-operation gap fields | Catalog adds PLOP duration, overhead, repeated-duration, radiated-bits, and command-radiation efficiency formulas. |
| COP1-016 | Section 5.1.9.2 timing budget | The T1 budget can be separated into acknowledgement-loop latency and margin above the configured `T1_Initial` value | Catalog adds `COP1_ACKLatencyBudget`, `COP1_T1_Margin`, and `COP1_TimeoutRate`. |
| COP1-017 | Sections 2.1, 2.2, and 5.1.10 | Go-back-N recovery retransmits the unacknowledged Sent_Queue while the first-frame Transmission_Count has not reached Transmission_Limit | Catalog adds maximum recovery-transmission count and recovery-duration upper-bound formulas. |
| COP1-018 | Sections 5.1.4, 5.1.8, and 5.2.10 | Wait_Queue capacity, Sent_Queue acknowledgements, and FOP window availability determine whether more Type-AD work can enter the COP loop | Catalog adds Sent_Queue occupancy, window remaining, Wait_Queue-full, acknowledged-frame, `NN(R)` update, and timer-restart formulas. |
| COP1-019 | Sections 5.1.9 through 5.1.10 | Timeout or retransmit-request recovery stops when Transmission_Count reaches Transmission_Limit and the selected Timeout_Type path is invoked | Catalog adds first-order Type-AD goodput and abort-condition formulas. |
